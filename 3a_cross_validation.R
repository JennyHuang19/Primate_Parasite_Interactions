...
for (rr in 1  : repetitions) {
  ...
  mcmc <- MCMC_trimResults(obs_A = use_A, focus = obs_F2, occur_B = obs_OB2, occur_P = obs_OP2,
  ...
  pred <- mcmc$pL1s_mean # we took the mean.
  save(pred, file = paste0(save_path, 'predmay13_', rr, '.dat'))
  rm(pred)
  
  
  save(cv_indices, file = paste0(save_path, 'cv_indicesmay13_', rr, '.dat'))
  rm(cv_indices)
}
