...
###
library (readr)
urlfile="https://raw.githubusercontent.com/gpapadog/Bird_Plant_Interactions/master/Data/ATLANTIC_frugivory.csv"
###

#dta <- fread(paste0(data_path, 'ATLANTIC_frugivory.csv'))
dta <-read_csv(url(urlfile)) # JH: 8319 rows.
...
primate_traits <- primate_traits_original %>% 
  select(HostCorrectedName, diel_activity, bodymass_kg, habitat_category, SSD, locomotion) # (LATER: add trophic, iucn, mating system, and more.)
...
NOT NEEDED FOR THE PRIMATE STUDY.
...
```{r}
pM <- c(2, 3)  # Two continous and three binary covariates:
obs_X2 <- matrix(NA, nrow = nM, ncol = sum(pM))
rownames(obs_X2) <- uni_primates
colnames(obs_X2) <- c('logBodyMass', 'logSSD', 'isnocturnal', 'isarboreal', 'isforest')
obs_X2[, 1] <- log(all_X2$bodymass_kg) # taking logs of body mass
obs_X2[, 2] <- log(all_X2$SSD)
obs_X2[, 3] <- (all_X2$diel_activity == 'nocturnal') * 1
obs_X2[, 4] <- (all_X2$locomotion == 'arboreal') * 1
obs_X2[, 5] <- (all_X2$habitat_category == 'forest') * 1 # can we change to an ordinal variable?
obs_X2
...
```
Out of our 217 studies, we have 49 that study multiple hosts, although some study only one genera. I'll select those studies to be parasite oriented.
...
network_studies <- intersect(multihost_studies, multibug_studies) %>% 
  distinct(Citation) %>% 
  select(Citation) # this turns df into a list of citations
