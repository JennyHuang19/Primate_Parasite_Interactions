...
#' @export
#' 
MCMC_trimResults <- function(obs_A, focus, occur_B, occur_P, obs_X, obs_W, Cu, Cv,
  ...
  # If the probabilities of occurrence for the first set of species is either 0
  # or 1, we never have to update their occurrence indicator.
  # if (length(unique(as.numeric(occur_B))) <= 2) { # (may 17th) check if O_B is 0/1-binary, not if it has 2 unique values. length(vals(0)) + length(vals(1)) == length(entire array)
  #   sampling$O_B <- FALSE
  # }
  if (all(as.numeric(occur_B) == 0 | as.numeric(occur_B) == 1)){
    sampling$O_B <- FALSE
  }
  ...
  #   sampling$O_P <- FALSE
  # }
  if (all(as.numeric(occur_P) == 0 | as.numeric(occur_P) == 1)){
    sampling$O_P <- FALSE
  }
  ...
  cum_sum_Us <- matrix(data = 0, nrow = nB, ncol = use_H) # (may 24th)
  cum_sum_Vs <- matrix(data = 0, nrow = nP, ncol = use_H) # (may 24th)
  ...
  cum_sum_pis <- matrix(data = 0, nrow = nB) # (may 24th)
  cum_sum_pjs <- matrix(data = 0, nrow = nP) # (may 24th)
  ...
  for (ss in 1 : (Nsims * thin + burn)) {
    ...
    if ((ss - burn) %% thin == 0 & ss > burn) {
      ...
      # (may 24th) --
      cum_sum_Us <- cum_sum_Us + this_U
      Us_mean <- cum_sum_Us/keep_index
      cum_sum_Vs <- cum_sum_Vs + this_V
      Vs_mean <- cum_sum_Vs/keep_index
      # ---
      vs[keep_index, ] <- this_v
      ...
      # (may 24th) --
      cum_sum_pis <- cum_sum_pis + this_pi
      pis_mean <- cum_sum_pis/keep_index
      cum_sum_pjs <- cum_sum_pjs + this_pj
      pjs_mean <- cum_sum_pjs/keep_index
      # ---
  ...
  r <- list(Ls = Ls, pL1s = pL1s, mod_pL1s = mod_pL1s,
            ...
            # Us = Us, Vs = Vs, 
            Us_mean = Us_mean, Vs_mean = Vs_mean,
            vs = vs, omegas = omegas, zs = zs,
            thetas = thetas, 
            # pis = pis, pjs = pjs, 
            pis_mean = pis_mean, pjs_mean = pjs_mean,
            rU = rU, rV = rV,
